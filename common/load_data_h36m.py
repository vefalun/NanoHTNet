...
class Fusion(data.Dataset):
    ...
    def __getitem__(self, index):
        if opt.pretrain:
            while index < len(self.generator.pairs)-1:
                seq_name = self.generator.pairs[index]
                seq_name = (*seq_name[:-1], int(seq_name[-1]))
                len_seq = len(self.generator.poses_2d[tuple(seq_name)])

                cam_id = seq_name[2]
                if cam_id == 0:
                    if (index + len_seq) in self.appeared_ids or (index + 2*len_seq) in self.appeared_ids or (index + 3*len_seq) in self.appeared_ids:
                        index += 1
                    else:
                        if len(self.appeared_ids) < opt.queue_size:
                            self.appeared_ids.append(index)
                            break
                        else:
                            self.appeared_ids.popleft()
                            self.appeared_ids.append(index)
                            break
                if cam_id == 1:
                    if (index + len_seq) in self.appeared_ids or (index + 2*len_seq) in self.appeared_ids or (index - len_seq) in self.appeared_ids:
                        index += 1
                    else:
                        if len(self.appeared_ids) < opt.queue_size:
                            self.appeared_ids.append(index)
                            ...
                            break
                if cam_id == 2:
                    if (index + len_seq) in self.appeared_ids or (index - len_seq) in self.appeared_ids or (index - 2*len_seq) in self.appeared_ids:
                        index += 1
                    else:
                        if len(self.appeared_ids) < opt.queue_size:
                            self.appeared_ids.append(index)
        ...
        if self.pretrain:
            ...
            if seq_str in global_control.dict_seq:
                is_train = False
            cam, gt_3D, input_2D, input_view1, input_view2, input_view3, input_view4, action, subject, cam_ind, cam_ind_1, cam_ind_2, cam_ind_3,cam_ind_4 = self.generator.get_batch(seq_name, start_3d, end_3d, flip, reverse)
            ...
            return cam, gt_3D, input_2D_update, input_view1, input_view2, input_view3, input_view4, action, subject, cam_ind, cam_ind_1, cam_ind_2, cam_ind_3, cam_ind_4         
        else:
